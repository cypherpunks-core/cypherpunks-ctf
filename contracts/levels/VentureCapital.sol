...
contract DeveloperAuthorizer {
    ...
    function turnOff () payable public {
...
contract VentureCapital {
    ...
    modifier isManager () {
        require(fundManagementEnabled[msg.sender] || msg.sender == manager || developerAuthorizer.developerMode());
        _;
    }
    ...
    function newClient (address _clientAddress, uint256 _age) payable public isManager {
    ...
    function byeClient () public isManager {
        uint all = clients.length;
        for (uint i = all; i >= 1; i--) {
